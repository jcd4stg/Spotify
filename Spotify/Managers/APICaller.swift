...
final class APICaller {
    ...
    public func getCurrentUserPlaylist(completion: @escaping ((Result<[Playlist], Error>) -> Void)) {
         createRequest(
            with: URL(string: Constants.baseAPIURL + "/me/playlists?limit=5"),
            type: .GET
         ) { request in
    ...
    public func getRecommendations(genres: Set<String>, completion: @escaping ((Result<RecommendationsResponse, Error>) -> Void)) {
        ...
        createRequest(with: URL(
            string: Constants.baseAPIURL + "/recommendations?limit=40&seed_genres=\(seeds)"),
                      type: .GET
        ) { request in
    ...
    // MARK: - Search
    public func search(with query: String, compleiton: @escaping ((Result<[SearchResult], Error>) -> Void)) {
        createRequest(
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else {
                    compleiton(.failure(APIError.failedToGetData))
                    return
                }
                ...
                do {
                    ...
                    compleiton(.success(searchResult))
